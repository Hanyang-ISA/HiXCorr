   ...
   for (z=zStart; z<zStop; z++)
   {
               ...
               && iPrecursorCharge<=g_staticParams.options.iEndCharge ) )
      {
               ...
               && iPrecursorCharge <= g_staticParams.options.iMaxPrecursorCharge)
         {
			...
			// g_staticParams.options.bSparseMatrix
			// g_staticParams.tolerances.dFragmentBinSize
			if (g_staticParams.options.bHiXCorr && g_staticParams.tolerances.dFragmentBinSize < 0.1) {
				if (!PreprocessHiXCorr(pScoring, spec, pdTmpRawData, pdTmpFastXcorrData,
...
void CometPreprocess::GetTopIonsHiXCorr(map< int, double > *pdTmpRawData,
struct msdata *pTmpSpData,
	int iArraySize)
{
	int  i,
...
bool CometPreprocess::LoadIonsHiXCorr(struct Query *pScoring,
									  map<int, double> *pdTmpRawData,
									  map<int, double> *tmpSepcData,
									  Spectrum mstSpectrum,
									  struct PreprocessStruct *pPre)
{
	int  i;
	...
	double *pdTmpPeakExtracted)
{
	...
	if (!LoadIonsHiXCorr(pScoring, &tmpXcorrData, &tmpSpecData, mstSpectrum, &pPre))
	{
		return false;
	}
	...
	GetTopIonsHiXCorr(&tmpSpecData, &(pTmpSpData[0]), pScoring->_spectrumInfoInternal.iArraySize);
	qsort(pTmpSpData, NUM_SP_IONS, sizeof(struct msdata), QsortByIon);
